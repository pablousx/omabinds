(() => {
  ...
  try {
    applyTheme(localStorage.getItem("omabinds-theme"));
  } catch {
    applyTheme("night");
  }
  ...
  themeButton.addEventListener("click", () => {
    ...
    try {
      localStorage.setItem("omabinds-theme", root.dataset.theme);
    } catch {
      /* Optional preference. */
    }
  });
  ...
  if (copy && navigator.clipboard && window.isSecureContext) {
    ...
    copy.addEventListener("click", async () => {
      ...
      try {
        ...
        status.textContent = "Comando de instalación copiado.";
      } catch {
        status.textContent =
          "No se pudo copiar. Selecciona el comando manualmente.";
      }
    });
  }
})();
